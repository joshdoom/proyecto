...
def screen_grado_iv(tk: tkinter, window: Tk):
    ...
    def update_table():
        ...
        with Session(engine) as session:
            ...
            for estudiante in estudiantes:
                grado = session.query(ModelGrado).filter(ModelGrado.id_estudiante == estudiante.id, ModelGrado.inscrito == 4).first()
    ...
    def nuevo():
        ...
        with Session(engine) as session:
            ...
            connect_grado.create(4, estudiante.id)
    ...
    def guardar():
        ...
        with Session(engine) as session:
            ...
            connect_grado.update(grado.id, 4, estudiante.id)
    ...
    def notas():
        from ..notas import screen_notas
        window.destroy()
        screen_notas(tkinter, window=tk.Toplevel(), degree=4)
        
    window.title("Cuarto año")
